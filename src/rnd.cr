...
module Rnd
  ...
  private class RandomGaussian
    ...
    initializer mean : Float64, stddev : Float64, rand_helper : Random
    ...
    private def gaussian(mean, stddev, rand_helper)
      theta = 2 * Math::PI * rand_helper.rand
      rho = Math.sqrt(-2 * Math.log(1 - rand_helper.rand))
      scale = stddev * rho
      tmp = mean + scale
      {tmp * Math.cos(theta), tmp * Math.sin(theta)}
    end
  end

  private GAUSSIAN = RandomGaussian.new(0.0, 1.0, Random::Secure)
