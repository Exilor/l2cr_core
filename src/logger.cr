# Crystal's former Logger class with some modifications.
class Logger
  ...
  # :nodoc:
  record Message, severity : Severity, datetime : Time, progname : String,
    message : String
  ...
  {% for name in Severity.constants %}
    ...
    def {{name.id.downcase}}(message)
      log(Severity::{{name.id}}, nil, message)
    end
    ...
    def {{name.id.downcase}}(progname, message)
      log(Severity::{{name.id}}, progname, message)
    end
  ...
  def log(severity, message)
    return if severity < level || @io.nil?
    write(severity, Time.local, @progname, message)
  end
  ...
  def log(severity, progname, message)
    return if severity < level || @io.nil?
