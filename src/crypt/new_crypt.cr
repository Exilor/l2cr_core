...
struct NewCrypt
  private STATIC_BLOWFISH_KEY = UInt8.slice(
    107, 96, 203, 91, 130, 206, 144, 177, 204, 43, 108, 85, 108, 108, 108, 108
  )

  def initialize(key = STATIC_BLOWFISH_KEY)
  ...
  def encrypt(data, offset : Int32, size : Int32)
    pos = offset
    stop = offset &+ size
    while pos < stop
  ...
  def decrypt(data, offset : Int32, size : Int32)
    pos = offset
    stop = offset &+ size
    while pos < stop
  ...
  def self.verify_checksum(data, offset : Int32, size : Int32) : Bool
     return false if size & 3 != 0 || size <= 4
  ...
  def self.append_checksum(data, offset : Int32, size : Int32)
    checksum = 0
    count = size &- 4
    ecx, i = 0, offset
  ...
  def self.xor(data, offset : Int32, size : Int32, key : Int32)
    stop = size &- 8
    pos = 4 &+ offset
    edx, ecx = 0, key
